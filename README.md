...
该组件基于 [`tymon/jwt-auth`](https://github.com/tymondesigns/jwt-auth ) 和 [`hyperf-ext/jwt`](https://github.com/hyperf-ext/jwt)，实现了完整用于 JWT 认证的能力，支持 hyper 3.0。

## 安装

```shell script
composer require youbuwei/hyperf-jwt
```

## 发布配置

```shell script
php bin/hyperf.php vendor:publish youbuwei/hyperf-jwt
```
...
[
    ...
    */
    ...
    | 指定用于实现在黑名单中存储令牌行为的类。
    |
    | 自定义存储类需要实现 `Youbuwei\HyperfJwt\Contracts\StorageInterface` 接口。
    |
    */
...
class SomeClass
{
     ...
     *
     * @var Youbuwei\HyperfJwt\Contracts\ManagerInterface
     */
    protected $manager;
